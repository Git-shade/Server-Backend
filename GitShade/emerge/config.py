...
from GitShade.emerge.graph import GraphType
...
class Configuration:
    ...
    def setup_commang_line_arguments(self) -> None:
        ...
        print(f'tmp/{shared_module.get_variable()}/extracted/emerge.yaml')
        emerge_file = f'tmp/{shared_module.get_variable()}/extracted/emerge.yaml'
        print('The emerge file is ---- ',emerge_file)
        self.arg_parser = argparse.ArgumentParser(description='🔎 Welcome to emerge ' + f'{self.version}')
        self.arg_parser.add_argument('-c', '--config', dest='yamlconfig', default = emerge_file, help='set yaml config file')
